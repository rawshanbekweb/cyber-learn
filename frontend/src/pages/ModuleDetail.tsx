...
import { Shield, Wifi, Lock, Server, ArrowLeft, Terminal, AlertTriangle, CheckCircle2, XCircle, Download, Loader2, Flag } from "lucide-react";
  ...
  return (
    <Card className="bg-card/40 backdrop-blur border-border/50">
      ...
      <CardContent className="space-y-4">
        {challenges.map(ch => (
          <CTFChallengeCard key={ch.id} challenge={ch} token={token} />
        ))}
      </CardContent>
    </Card>
  );
